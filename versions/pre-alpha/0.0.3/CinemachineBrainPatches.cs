...
namespace ThirdPersonCamFix
{
    ...
    internal static class CinemachineBrainPatches
    {
        ...
        private static void LateUpdate_Prefix(CinemachineBrain __instance)
        {
            ...
            {
                ThirdPersonCamFixPlugin.LogSource?.LogError("[TPCF] (PATCH) Failed to apply shoulder offset: " + ex);
            }
        }
        ...
        private static bool TryApplyCameraManagerOffset(float blend)
        {
            ...
            if (clamped >= 0f)
            {
                // Restore original right-shoulder offset exactly.
                desiredX = baseX;
                desiredSX = baseSX;
            }
            else
            {
                // Mirror the original magnitude if present; otherwise keep zero.
                if (hasNativeOffset)
                {
                    desiredX = -Mathf.Abs(baseX);
                }
                else
                {
                    desiredX = baseX; // keep zero if game uses screenX-only offsetting
                }
                desiredSX = 1f - baseSX;
            }
        ...
        {
            if (ThirdPersonCamFixPlugin.LogSource == null)
                return;
            ...
            ThirdPersonCamFixPlugin.LogSource.LogInfo(
                $"[TPCF] framing baseX={baseX:F3} curX={currentX:F3} desX={desiredX:F3} baseSX={baseSX:F3} curSX={currentSX:F3} desSX={desiredSX:F3} side={side}"
            );
        }
